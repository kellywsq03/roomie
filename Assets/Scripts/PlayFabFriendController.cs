...
namespace KnoxGameStudios
{
    public class PlayfabFriendController : MonoBehaviour
    {
        ...
        private void HandleAddPlayfabFriend(string name)
        {
            ...
            PlayFabProfileModels.AddFriend(request, OnFriendAddedSuccess, OnFailure);
        }
        private void HandleRemoveFriend(string name)
        {
            string id = friends.FirstOrDefault(f => f.Username == name).FriendPlayFabId;
            Debug.Log($"Playfab remove friend {name} with id {id}");
            var request = new RemoveFriendRequest { FriendPlayFabId = id };
            PlayFabProfileModels.RemoveFriend(request, OnFriendRemoveSuccess, OnFailure);
        }
        ...
        private void GetPlayfabFriends()
        {
            ...
            PlayFabProfileModels.GetFriendsList(request, OnFriendsListSuccess, OnFailure);
        }
        #endregion
